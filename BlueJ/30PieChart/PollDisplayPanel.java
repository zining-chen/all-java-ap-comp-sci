...
public class PollDisplayPanel extends JPanel
{
    ...
    private int count1, count2, count3, count4; // Declare the int fields count1, count2, count3:
    ...
    private void drawPieChart(Graphics g, int x, int y, int r)
    {
        ...
        if (total > 0)
        {
            ...
            //System.out.println ( "Red: " + fromDegree + "  " + degrees);

            g.setColor(Color.GREEN);
            fromDegree = degrees;
            degrees =  countToDegrees(count2, total);
            drawSector(g, x, y, r, fromDegree, degrees);
            //System.out.println ( "Green: " + fromDegree + "  " + degrees);

            g.setColor(Color.BLUE);
            fromDegree = degrees + fromDegree;
            degrees = countToDegrees(count3, total);
            drawSector(g, x, y, r, fromDegree, degrees);
            //System.out.println ( "Blue: " + fromDegree + "  " + degrees);

            g.setColor(Color.ORANGE);
            fromDegree = fromDegree + degrees;
            degrees = Math.max(360 - fromDegree,0);
            drawSector(g, x, y, r, fromDegree, degrees);
            //System.out.println ( "Orange: " + fromDegree + "  " + degrees);
    ...
    private void drawLegend(Graphics g, int x, int y, int r)
    {
        ...
        g.drawString( count1 + "" , x - 3*r/2, y);

        g.drawString( count2 + "" , x - r/2, y);

        g.drawString( count3 + "" , x + r/2, y);

        g.drawString( count4 + "" , x + 3*r/2 , y);

        // Display the color squares:
        y += 5;
        x -= 2;
        g.setColor(Color.RED);
        g.fillRect(x - 3*r/2, y, 10, 10);
        g.setColor(Color.GREEN);
